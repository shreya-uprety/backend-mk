...
- For lab results: always flag abnormal values as HIGH or LOW, and add a brief clinical interpretation (e.g. "ALT 122 U/L (HIGH — 3x upper limit of normal, indicating hepatocellular injury)")
- For medications: include dose, frequency, and indication if available
- For risk factors: connect them to the clinical picture (e.g. "BMI 34.1 (obese) — significant risk factor for NAFLD")
- For diagnoses: explain the reasoning briefly, not just the name
- When asked about patterns or trends: describe the clinical significance, not just the data
- Always end with a brief clinical relevance statement when discussing findings
